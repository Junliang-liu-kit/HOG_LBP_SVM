...
# 训练集图片的位置
train_image_path = 'image_1000_128'
# 测试集图片的位置
test_image_path = 'image_test128'



# 训练集标签的位置
train_label_path = os.path.join('image_1000_128','train.txt')
# 测试集标签的位置
test_label_path = os.path.join('image_test128','test.txt')
...
# 提取特征并保存
def get_feat(image_list, name_list, label_list, savePath, feature_name):
    i = 0
    for image in image_list:
        ...
        #使用LBP方法提取图像的纹理特征
        lbp = local_binary_pattern(gray, n_point, radius, 'default')
        #统计图像的直方图
        max_bins = int(lbp.max() + 1)
        #hist size:256
        lbp_hist, _ = np.histogram(lbp, density=True, bins=max_bins, range=(0, max_bins))
        lbp_hist = np.concatenate((lbp_hist, [label_list[i]])) #lbp_hist feature + 它对应的label
        
        lbp_name = name_list[i] + '.feat'
        lbp_path = os.path.join(savePath, lbp_name)
        joblib.dump(lbp_hist, lbp_path)
        i += 1
    print(f"{feature_name} features are extracted and saved.")


# 变成灰度图片
def rgb2gray(im):
    gray = im[:, :, 0] * 0.2989 + im[:, :, 1] * 0.5870 + im[:, :, 2] * 0.1140
    #gray = np.sqrt(gray) #Gamma校正
    return gray
...
def get_name_label(file_path):
    ...
    with open(file_path) as f:
        for line in f.readlines():
            ...
            if len(line)>=3: 
                name_list.append(line.split(' ')[0])                                    #txt文件中 每行信息 第一部分加入name_list
                label_list.append(line.split(' ')[1].replace('\n','').replace('\r','')) #txt文件中 每行信息 第一部分加入label_list,且label必须为数字
...
def extra_feat():
    ...
    get_feat(train_image, train_name, train_label, train_feat_path, feature_name='train')
    get_feat(test_image, test_name, test_label, test_feat_path, feature_name='test')
...
def train_and_test():
    ...
    for feat_path in glob.glob(os.path.join(train_feat_path, '*.feat')):
        ...
        features.append(data[:-1])   #读取所有图片所有feature
        labels.append(data[-1])      #读取所有图片所有label
    ...
    for feat_path in glob.glob(os.path.join(test_feat_path, '*.feat')):
        ...
        data_test_feat = data_test[:-1].reshape((1, -1)).astype(np.float64) #转化成1行
...
if __name__ == '__main__':
    ...
    if sys.version_info < (3,):                                  # 判断python是否为3.0以上（包括3.0）
